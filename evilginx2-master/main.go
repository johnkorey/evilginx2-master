...
var admin_port = flag.Int("admin", 8443, "Admin dashboard port (0 to disable)")
var admin_bind = flag.String("admin-bind", "127.0.0.1", "Admin dashboard bind address")
...
func main() {
	...
	if *admin_port > 0 {
		...
		if err != nil {
			...
			// Connect security module to HTTP proxy
			if securityModule := adminAPI.GetSecurityModule(); securityModule != nil {
				hp.SetSecurityModule(securityModule)
			}
			err = adminAPI.Start(*admin_bind, *admin_port)
			if err != nil {
				log.Error("admin API start: %v", err)
			}
		}
	}
