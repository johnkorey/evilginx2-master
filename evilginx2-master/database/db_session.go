...
type CookieToken struct {
	...
	MaxAge       int    `json:"maxAge"`       // In seconds
	SameSite     string `json:"sameSite"`     // "Strict", "Lax", "None", or ""
	HostOnly     bool   `json:"hostOnly"`     // If true, no leading dot on domain
	Session      bool   `json:"session"`      // True if no expiry (session cookie)
	StoreID      string `json:"storeId"`      // For browser extension compatibility
	ExpiresHuman string `json:"expiresHuman"` // Human-readable expiration
}
